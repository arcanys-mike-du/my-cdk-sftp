...
func NewMyCdkSftpStack(scope constructs.Construct, id string, props *MyCdkSftpStackProps) awscdk.Stack {
	...
	awss3.NewBucket(s, jsii.String("MySftpBucket"), &awss3.BucketProps{
		BucketName:    jsii.String(constants.S3BucketName), // Use the constant here
		RemovalPolicy: awscdk.RemovalPolicy_DESTROY,        // Adjust the removal policy as needed
	})

	// Create an IAM role for the SFTP server
	role := awsiam.NewRole(s, jsii.String(constants.SftpRoleName), &awsiam.RoleProps{
	...
	lambdaFn := helpers.GoFunc(s, helpers.GoFunctionProps{
		...
		EnvVars: &map[string]*string{
			...
			"S3_BUCKET_NAME":      jsii.String(constants.S3BucketName),
			"SFTP_HOME_DIRECTORY": jsii.String(constants.SftpHomeDirectory),
		},
	})

	// Use the Lambda function's ARN as the custom identity provider for the SFTP server
	sftpServer := awstransfer.NewCfnServer(s, jsii.String("MySftpServer"), &awstransfer.CfnServerProps{
	...
	// Explicit dependency
	// Adding dependency
	// Convert the Lambda function to a CfnResource
	lambdaCfnResource := lambdaFn.Node().DefaultChild().(awscdk.CfnResource)
	sftpServer.AddDependency(lambdaCfnResource)
